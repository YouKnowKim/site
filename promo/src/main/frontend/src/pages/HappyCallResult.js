...
import HappyCallDetailModal from '../components/modal/HappyCallDetailModal.js';
...
const HappyCallResult = () => {
  ...
  const fetchAgencyList = async () => {
    try {
      ...
      const response = await axios.get('/api/promo/getHappyAgencyList', {
        params: {
  ...
  const handleExcelDownload = () => { 
    ...
    const fileName = `해피콜결과_${today}.xlsx`;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // ✅ API 호출 - 변경된 데이터만 전송
      const response = await axios.post('/api/promo/saveHappyCall', dataToSave);
      ...
      <Card>
        <Card.Header className="bg-light text-dark fw-bold" style={{ padding: '9px 15px' }}>
          <Row className="align-items-center">
            {/* 왼쪽: 제목 */}
            <Col xs="auto">
              <span style={{ fontSize: '16px', fontWeight: 'bold' }}>해피콜 결과</span>
            </Col>
...
export default HappyCallResult;